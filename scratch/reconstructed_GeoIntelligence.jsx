...
function getBacklogClearance(pendingQty = 0, dailyAvgQty = 0) {
  const pending = Number(pendingQty) || 0;
  const dailyAvg = Number(dailyAvgQty) || 0;
  
  if (pending <= 0) {
    return { days: 0, status: 'LOW', text: 'Clear' };
  }
  if (dailyAvg <= 0) {
    return { days: null, status: 'LOW', text: 'N/A' };
  }
  
  const days = pending / dailyAvg;
  
  let status = 'LOW';
  if (days > 10) status = 'CRITICAL';
  else if (days >= 5) status = 'HIGH';
  else if (days >= 2) status = 'MEDIUM';
  
  return {
    days,
    status,
    text: `${days.toFixed(1)} ${days === 1 ? 'day' : 'days'}`
  };
}

// ─── Tooltip (fixed-positioned, follows mouse) ────────────────────────────────
function Tooltip({ tooltipRef, visible, name, data, filterType, totalPending }) {
  if (!visible || !name) return null;
  
  const isPending = filterType === "PENDING";
  const pendingQty = data ? (data.pendingQty ?? data.volume ?? 0) : 0;
        ...
        <div className="space-y-1.5">
          ...
          {isPending ? (
            <>
              <Row 
                label="Share" 
                value={totalPending > 0 ? `${((pendingQty / totalPending) * 100).toFixed(1)}%` : "0.0%"} 
                valueColor="#3b82f6" 
              />
...
export default function GeoIntelligence({ salesData: propSalesData }) {
  ...
  }, [rawData, filterState.type, selectedMonth, monthButtons]);
  ...
  }, [propSalesData, rawData, monthButtons, filterState, totalVolume, selectedMonth]);
  ...
  const totalPendingVolume = useMemo(() => {
    ...
    return rankedList.reduce((sum, item) => sum + (item.volume || 0), 0);
  ...
  const { heatColorScale, geoHeatMap } = useMemo(() => {
    ...
    if (filterState.type === "PENDING") {
      entries.forEach(([key, e]) => {
        const pendingQty = e.pendingQty !== undefined ? e.pendingQty : (e.volume || 0);
        heatMap[key] = {
    ...
    return { heatColorScale: colorScale, geoHeatMap: heatMap };
  }, [activeMap, filterState.type, selectedMonth, monthButtons]);
  ...
  useEffect(() => {
    ...
    const drawMap = () => {
      ...
      allPaths.attr("fill", d => {
        ...
        if (entry) {
          if (filterState.type === "PENDING") {
            const pendingQty = entry.pendingQty || 0;
  ...
  return (
    <div>
        ...
        >
          ...
          <div className="absolute bottom-3 left-3 rounded-lg px-3 py-2 border"
            ...
            {filterState.type === "PENDING" ? (
              <Legend 
                colors={['#ffffff', '#fdba74', '#f97316', '#ef4444']} 
          ...
          <div className="rounded-xl border p-4 flex-shrink-0" style={{ background: '#161b22', borderColor: '#1e293b' }}>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <StatCard
                ...
                value={formatNumber((selectedState ? rankedDistricts : rankedStates)
            ...
            <div className="flex-1 overflow-y-auto pr-1">
              {rankedList.map((item, idx) => {
                ...
                const trendVal = isPending
                  ? (totalPendingVolume > 0 ? `${((item.volume / totalPendingVolume) * 100).toFixed(1)}%` : '0.0%')
                  : trendStr(item.trend);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 text-right">
                      <span className="text-slate-300">{formatMT(item.volume)}</span>
      ...
      {/* Floating tooltip */}
      <Tooltip tooltipRef={tooltipRef} {...tooltip} filterType={filterState.type} totalPending={totalPendingVolume} />
    </div>
  );
}
